...
def main(filename, start, count, output):
    ...
    with h5py.File(filename, mode='r') as file:
        ...
        for index in range(start, start+count):
            ...
            fc = cmap(norm(data))
            Tc = 0.9760843770270632
            lat_c = np.arccos(Tc**4)
            #print(theta)
            ind_theta_c = np.abs(lat-lat_c).argmin()
            ind_mtheta_c = np.abs(lat+lat_c).argmin()
            fc[:, theta.size//2, :] = [0,0,0,1]  # black equator
            fc[:, ind_theta_c, :] = [0,0,1,1]  # black equator
            fc[:, ind_mtheta_c, :] = [0,0,1,1]  # black equator
            if index == start:
