...
def main(filename, start, count, output):
    ...
    with h5py.File(filename, mode='r') as file:
        ...
        for index in range(start, start+count):
            ...
            vmax = 140#np.max(np.abs(data))
            data[data < -vmax] = - vmax
            data[data > vmax]  =   vmax
            contf = plt.contourf(phi_mat,lat_mat,np.transpose(data),transform=ccrs.PlateCarree(),cmap=cmap,levels=np.linspace(-vmax,vmax,60))
            ax.gridlines(draw_labels=True, linewidth=2, color='gray', alpha=0.5, linestyle='--')
            plt.colorbar(contf,ticks=[-vmax,-vmax//2,0,vmax//2,vmax],pad=0.1,fraction=0.1,aspect=10)
